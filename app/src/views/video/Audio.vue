<template>
  <div class="html_body">
    ...
    <div id="codeCT" class="audio_ct">
      <cube-scroll ref="audioScroll" :scroll-events="scroll.audio.events" @pulling-down="audioDown" :options="scroll.audio.options">
        <!-- <cube-swipe> -->
          <div class="audio_img" @click="selectImg()">
          ...
          <cube-form :model="formData" @validate="validate" :immediateValidate="true" class="cube-form_groups">
            <cube-form-group legend="录制音频">
              <ul class="upload">
                <li class="upload_img" v-for="(val,index) in formData.upload" :key="index">
                  <i @click="removeAudio(index)" v-if="val.del||val.loading==0">x</i>
                  <div v-if="!val.src">{{val.loading}}%</div>
                  <audio v-if="val.src" :src="val.src" conrtols="true" />
                </li>
                <li class="upload_an" @click="addAudio()">
                  <span v-if="!audio">+</span>
                  <span v-else style="color: #FF6600;">■</span>
                </li>
              </ul>
            </cube-form-group>
...
.top_header{background-color: #FF6600;}
.audio_ct{height: calc(100% - 66px); overflow-y: auto;}

.audio_img{text-align: center; background-color: #FFF; background-size: 100% auto; background-repeat: no-repeat;}
.audio_img img{width: 100%;}
...
.upload_img audio{float: left; width: 100%; height: 100%;}
...
.sub{width: calc(100% - 20px); margin: 0 auto; text-align: center; padding: 15px 0; background: #4a4c5b; color: #FF6600; border-radius: 5px;}
.sub:active{background-color: #FF6600;}
</style>
