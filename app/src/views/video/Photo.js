...
export default {
  ...
  data(){
    return {
      ...
      // 滑动效果
      scroll:{
        photo:{events: ['scroll'],options:{pullDownRefresh: {threshold: 60,stop: 40,txt: '已更新'}}},
      },
      // 视频
      audio:null,
      // 表单数据
  ...
  methods:{
    ...
    photoDown(){
      // 加载数据
      this.$refs.photoScroll.forceUpdate();
    },
    ...
    submitData(){
      ...
      if(this.subStatus.dis && this.formData.upload.length>0 && this.formData.img){
        ...
        this.$ajax.post(
          ...
          'token='+this.$inc.token()+'&type=2&data='+data
        ).then(function(res){
          res = res.data;
          if(res.code!=0){
      ...
      _self.formData.upload[n-1].obj = this.$inc.camera(function(file,entry){
        ...
        _self.$inc.compressImage(entry.fullPath,{width: 1024, height: 768},function(imgBase64) {
          ...
          let data = [
            ...
            {type:'data',key:'up',val:imgBase64}
          ];
          // 上传视频
          _self.$inc.uploader(_self.$config.apiUrl+'UserVideo/photo',data,function(res){
    ...
    removePhoto(n){
      ...
      if(data[n].file){
        this.$ajax.post(
          this.$config.apiUrl+'UserVideo/photoDel',
