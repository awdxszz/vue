...
export default {
  ...
  data(){
    return {
      ...
      // 滑动效果
      scroll:{
        audio:{events: ['scroll'],options:{pullDownRefresh: {threshold: 60,stop: 40,txt: '已更新'}}},
      },
      // 音频
      audio:null,
      // 表单数据
  ...
  methods:{
    ...
    audioDown(){
      // 加载数据
      this.$refs.audioScroll.forceUpdate();
    },
    ...
    submitData(){
      ...
      if(this.formData.upload.length>0 && this.formData.img){
        ...
        this.$ajax.post(
          ...
          'token='+this.$inc.token()+'&type=1&data='+data
        ).then(function(res){
          res = res.data;
          if(res.code!=0){
    ...
    addAudio(){
      ...
      // 开始
        ...
        _self.formData.upload[n-1].loading = 1;
        // 录音
        try{
          _self.audio = plus.audio.getRecorder();
          _self.$inc.audio(_self.audio,function(file){
            // 表单数据
            let data = [
            ...
            // 上传视频
            _self.$inc.uploader(_self.$config.apiUrl+'UserVideo/audio',data,function(res){
    ...
    removeAudio(n){
      ...
      if(data[n].file){
        this.$ajax.post(
          this.$config.apiUrl+'UserVideo/audioDel',
