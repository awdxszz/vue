...
export default {
  ...
  data(){
    return {
      ...
      // 搜索、删除
      seaData:{isShow:false,form:{nickname:'',name:'',tel:''}},
  ...
  mounted(){
    let _self = this;
    // 动作菜单
    this.$refs.ajax.post('UserMain/getMenusAction','&url=WebUser',function(res){
      let d = res.data;
      if(d.code!==0){
  ...
  methods:{
    ...
    loadData(){
      ...
      this.$refs.ajax.post('WebUser/list',perm,function(res){
        let d = res.data;
        if(d.code!==0){
          _self.$message.error(d.msg);
        }else{
    ...
    openAction(action){
      if(action=='list'){
        this.seaData.form = {nickname:'',name:'',tel:''};
    ...
    subDel(){
      ...
      // 提交
      this.$refs.ajax.post('WebUser/del','&key='+id,function(res){
        let d = res.data;
        if(d.code!==0){
          _self.$message.error(d.msg);
        }else{
